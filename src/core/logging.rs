...
/// Icône de la fenêtre, dérivée du jeu d'icônes du dépôt.
///
/// `None` si le décodage échoue : une icône manquante ne doit pas empêcher l'ouverture de la
/// fenêtre.
#[must_use]
pub fn icone_application() -> Option<iced::window::Icon> {
    iced::window::icon::from_file_data(
        include_bytes!("../../assets/icons/candilog-icon-256.png"),
        None,
    )
    .ok()
}
