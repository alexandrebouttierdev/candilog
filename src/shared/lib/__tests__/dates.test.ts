...
describe("conversion de date", () => {
  ...
  it("fait l'aller-retour sans perte", () => {
    expect(toDisplayDate(toIsoDate("29-02-2024") as string)).toBe("29-02-2024");
  });
  ...
  it("refuse une saisie hors format", () => {
    for (const saisie of ["2026-08-20", "20/08/2026", "20 août 2026", "20-8-2026", ""]) {
      expect(toIsoDate(saisie)).toBeNull();
    }
  });
  ...
  it("laisse intacte une date déjà affichable qu'on ne sait pas relire", () => {
    ...
    expect(toDisplayDate("2026-08-20T10:00:00Z")).toBe("20-08-2026");
    expect(toDisplayDate("date inconnue")).toBe("date inconnue");
  });

  it("affiche une date longue comme les maquettes", () => {
    expect(toLongDate("2026-08-02")).toBe("02 août");
    expect(toLongDate("2026-08-02", true)).toBe("02 août 2026");
  });
});
...
describe("horodatage d'un entretien", () => {
  it("compose la date, l'heure et le décalage local", () => {
    ...
    const timestamp = toTimestamp("25-08-2026", "14:00");

    expect(timestamp).toMatch(/^2026-08-25T14:00:00[+-]\d{2}:\d{2}$/);
  });

  it("refuse une date ou une heure invalide", () => {
    expect(toTimestamp("31-02-2026", "14:00")).toBeNull();
    expect(toTimestamp("25-08-2026", "25:00")).toBeNull();
  });

  it("fait l'aller-retour vers les champs du formulaire", () => {
    const timestamp = toTimestamp("25-08-2026", "09:05") as string;
