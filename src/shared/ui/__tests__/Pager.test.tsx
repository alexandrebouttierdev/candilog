...
import { Pager, pageBounds } from "../Pager";

describe("bornes de page", () => {
  it("décrit la tranche affichée", () => {
    expect(pageBounds(2, 8, 15)).toMatchObject({ from: 9, to: 15, pageCount: 2 });
  });

  it("affiche 0–0 sur une collection vide plutôt que 1–0", () => {
    expect(pageBounds(1, 8, 0)).toMatchObject({ from: 0, to: 0 });
  });

  it("garde une page même sans élément, pour ne pas afficher « page 1 sur 0 »", () => {
    expect(pageBounds(1, 8, 0).pageCount).toBe(1);
  });

  it("borne la fin de page au total réel", () => {
    expect(pageBounds(2, 8, 12).to).toBe(12);
  });

  it("désactive les flèches aux extrémités", () => {
    expect(pageBounds(1, 8, 15)).toMatchObject({ hasPrev: false, hasNext: true });
    expect(pageBounds(2, 8, 15)).toMatchObject({ hasPrev: true, hasNext: false });
  });
});

describe("Pager", () => {
  it("annonce la tranche et le total", () => {
    render(<Pager page={1} pageSize={8} total={15} label="candidatures" onPageChange={vi.fn()} />);
  ...
  it("demande la page suivante sans jamais recevoir la collection", async () => {
    // Le composant ne connaît que `page`, `pageSize` et `total` : il ne peut donc pas
    // paginer côté client, ce qui est précisément la garantie recherchée.
    const onPageChange = vi.fn();
    render(<Pager page={1} pageSize={8} total={15} label="candidatures" onPageChange={onPageChange} />);
  ...
  it("marque la page courante pour les technologies d'assistance", () => {
    render(<Pager page={2} pageSize={4} total={15} label="candidatures" onPageChange={vi.fn()} />);
