...
describe("schéma des filtres de candidatures", () => {
  it("accepte un filtre entièrement vide", () => {
    ...
    expect(applicationFilterSchema.safeParse(EMPTY_FILTER_INPUT).success).toBe(true);
  });
  ...
  it("refuse une borne de période mal formée", () => {
    expect(
      applicationFilterSchema.safeParse({ ...EMPTY_FILTER_INPUT, start_date: "01/08/2026" }).success,
    ).toBe(false);
  });
  ...
  it("normalise l'entreprise non choisie en null", () => {
    expect(applicationFilterSchema.parse({ ...EMPTY_FILTER_INPUT, company_id: "" }).company_id).toBeNull();
  });
  ...
  describe("amplitude horaire", () => {
    ...
    it("refuse une borne non numérique ou hors limites", () => {
      for (const min_weekly_hours of ["abc", "0", "-2", "500"]) {
        expect(
          applicationFilterSchema.safeParse({ ...EMPTY_FILTER_INPUT, min_weekly_hours }).success,
