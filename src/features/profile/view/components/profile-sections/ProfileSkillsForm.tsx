...
export function ProfileSkillsForm({ id, value, onSubmit }: { id: string; value: Skill[]; onSubmit: (value: Skill[]) => Promise<unknown> }) {
  ...
  return (
    <form id={id} onSubmit={(event) => void form.handleSubmit((data) => onSubmit(data.items))(event)}>
      <RepeatList empty="Aucune compétence ajoutée" addLabel="Ajouter une compétence" onAdd={() => rows.append(emptySkill())}>
        {rows.fields.map((field, index) => (
          <ItemCard key={field.id} title={`Compétence ${index + 1}`} onRemove={() => rows.remove(index)}>
            <div className="grid gap-4">
              <ProfileField required label="Nom" registration={form.register(`items.${index}.name`)} error={form.formState.errors.items?.[index]?.name?.message} />
              <ProfileArea label="Description (facultatif)" registration={form.register(`items.${index}.description`)} error={form.formState.errors.items?.[index]?.description?.message} />
            </div>
          </ItemCard>
        ))}
      </RepeatList>
