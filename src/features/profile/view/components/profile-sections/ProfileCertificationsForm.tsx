...
export function ProfileCertificationsForm({ id, value, onSubmit }: { id: string; value: Certification[]; onSubmit: (value: Certification[]) => Promise<unknown> }) {
  ...
  return (
    <form id={id} onSubmit={(event) => void form.handleSubmit((data) => onSubmit(data.items))(event)}>
      <RepeatList empty="Aucune certification ajoutée" addLabel="Ajouter une certification" onAdd={() => rows.append(emptyCertification())}>
        {rows.fields.map((field, index) => {
          const errors = form.formState.errors.items?.[index];
          return (
            <ItemCard key={field.id} title={values[index]?.name || `Certification ${index + 1}`} onRemove={() => rows.remove(index)}>
              <div className="grid gap-4 sm:grid-cols-2">
                <ProfileField required label="Nom" registration={form.register(`items.${index}.name`)} error={errors?.name?.message} />
                <ProfileField label="Organisme" registration={form.register(`items.${index}.issuer`)} />
                <ProfileField label="Date" registration={form.register(`items.${index}.date`)} placeholder="AAAA-MM" />
                <ProfileField label="Lien" type="url" registration={form.register(`items.${index}.url`)} error={errors?.url?.message} />
                <div className="sm:col-span-2">
                  <ProfileArea label="Description (facultatif)" registration={form.register(`items.${index}.description`)} error={errors?.description?.message} />
                </div>
              </div>
            </ItemCard>
          );
