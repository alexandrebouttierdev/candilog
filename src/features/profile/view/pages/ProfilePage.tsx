...
function IdentityRows({
  ...
  identity: Identity;
  fields: "identity" | "objective" | "online";
}) {
  const rows =
    fields === "identity"
      ? [
          ...
          { label: "E-mail", value: identity.email || null },
          ...
          { label: "Âge", value: identity.age == null ? null : String(identity.age) },
        ]
      : fields === "objective"
        ? [
            { label: "Titre", value: identity.title },
            { label: "Résumé", value: identity.resume },
  ...
  return (
    <div className="px-[18px] pt-1 pb-3">
      {rows.map((row) => (
        <Row key={row.label} label={row.label} value={row.value} multiline={row.label === "Résumé"} />
      ))}
    </div>
  );
}
...
function Row({
  ...
  return (
    <div
      className={
        multiline
          ? "flex flex-col gap-1 border-b border-line py-[9px] last:border-b-0"
          : "flex items-center justify-between gap-3.5 border-b border-line py-[9px] last:border-b-0"
      }
    >
      ...
      <span
        className={
          multiline
            ? "whitespace-pre-wrap text-body font-medium text-ink"
            : "min-w-0 flex-1 truncate text-right text-body font-medium text-ink"
        }
      >
